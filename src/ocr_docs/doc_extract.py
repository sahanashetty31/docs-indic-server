...
def results_to_markdown(results):
    ...
    for page, content in results.items():
        ...
        if "error" in content:
            ...
            # 'Response' is empty or not provided in new format, so skip if empty
            ...
            # Preserve line breaks by replacing single newline with double newline
...
def process_pdf(pdf_file, pages_str, prompt, src_lang, tgt_lang):
    ...
    system_prompt = "Do not return any asterisk"  # Currently unused, keep if needed later
